...
exports.db_update_checked = function(request, response){
    ...
    if(queryData.checked == 1){
        db.query('UPDATE schedule SET check=? WHERE id=?', [0, queryData.id], function(error, result){
            if(error) throw error;
            response.writeHead(200);
            response.end();
        });
    }else{
        db.query('UPDATE schedule SET check=? WHERE id=?', [1, queryData.id], function(error, result){
            if(error) throw error;
            response.writeHead(200);
            response.end();
        });
...
exports.db_delete = function(request, response){
    let queryData = url.parse(request.url, true).query;
    db.query('DELETE FROM schedule WHERE date=? AND schedule=?',
            [queryData.dateId, queryData.schedule],
